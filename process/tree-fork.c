...
void fork_child(int pi, int level)
{
    ...
    if(pids[left(pi)] == 0)
    {
        level == LEVEL_MAX? process(left(pi)-(1<<LEVEL_MAX+1)+1) : fork_child(left(pi), level+1);
        exit(0);
    }
    ...
    if(pids[right(pi)] == 0)
    {
        level == LEVEL_MAX? process(right(pi)-(1<<LEVEL_MAX+1)+1) : fork_child(right(pi), level+1);
        exit(0);
    }
    else if(pids[right(pi)] > 0)
        printf("%*s (%d) Forked child %d (%d)\n", level, "", getpid(), 1, pids[right(pi)]);
